...
export default function RegistrationTable({ registrations, onCheckIn }: RegistrationTableProps) {
  return (
          ...
          <tbody>
            {registrations.map((registration) => (
              <tr key={registration.id} className="border-b border-white/8 last:border-none">
                ...
                <td className="px-4 py-4 text-white/62">{new Date(registration.created_at).toLocaleString()}</td>
                <td className="px-4 py-4 text-white/62">{registration.checked_in_at ? new Date(registration.checked_in_at).toLocaleString() : "Sin ingreso"}</td>
                <td className="px-4 py-4">
                  <Button
